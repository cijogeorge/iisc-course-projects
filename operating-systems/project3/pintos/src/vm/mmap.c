  ...
  for (i = 0; i != size; ++i)
    {
      ...
      /*for (off = 0; off != SLOT_SIZE; ++off)*/
        /*block_read (spte->swap->disk, spte->swap->sector, spte->fte->kpage);*/
        x = file_read_at (file, spte->fte->kpage, PGSIZE, PGSIZE * i);
      list_push_back (&map->pages, &spte->mmap_elem);
      spte->mmapped = true;
      spte->swap->mapped_file = file;
      spte->swap->file_offset = i;
      vaddr += PGSIZE;
      sector += PGSIZE;
    }
