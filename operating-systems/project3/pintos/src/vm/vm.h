...
#define SECTOR_ERROR (block_sector_t)-1
#define SECTOR_ZERO (block_sector_t)-2
...
struct swap_t
{
  ...
  struct file * mapped_file; /*used for file map*/
  int file_offset;
};

struct block *fs;    /* disk used by the file system */
struct block *swap;  /* disk used by swap slots */
...
struct spte_t *vm_page_create (uint32_t *pd, void *vaddr, struct block *disk, block_sector_t sector);
