...
static block_sector_t find_free_slot (void);
...
{
  lock_init (&swap_lock);
  fs = block_get_role (BLOCK_FILESYS);
  swap = block_get_role (BLOCK_SWAP);
  if (swap)
    free_slots = bitmap_create (block_size (swap));
  else
    free_slots = NULL;
}
  ...
  if (pagedir_is_dirty (page->spde->pd, page->upage)) /* if page is dirty */
    {
      for (off = 0; off != SLOT_SIZE; ++off)
        block_write (page->swap->disk, page->swap->sector + off, kpage + PGSIZE * off / SLOT_SIZE);
    }
...
vm_load_page (struct spte_t *page)
{
  ...
  if (page->swap->sector != SECTOR_ZERO)
    {
      for (off = 0; off != SLOT_SIZE; ++off)
        block_read (page->swap->disk, page->swap->sector + off, page->fte->kpage + PGSIZE * off / SLOT_SIZE);
    }
  else /* All zero page */
